...
One rig, three readings, no scene change and no tailwater: `d_c` from the
...
where the Froude number passes through one:
    ...
    d_c = (q²/g)^⅓        E_c = 1.5·d_c        Fr = V/√(g y) = 1  at  y = d_c

A broad-crested weir is built to make that happen on purpose: the crest is a
control, so it should ride `d_c` — that is the assumption behind
...
hydrostatic, and `d_c = (q²/g)^⅓` — which assumes it is — stops applying. The
classical figure for the brink depth is 0.715·d_c.
...
(`level = 0.935 + 1.65·d_c + 0.03`); set both.
...
1. Set **Inflow q** and **Reservoir level** to your row — they are paired, not
   independent. Write down the `d_c` the q slider prints: that is reading
   **(1)**.
...
4. Reading **(3)**, at the lip: zoom in (wheel) on the right-hand end of the
   ...
   box. Submit **q, d_c, d_crest, d_brink**.

## For the instructor — pooling the class

Collect one row per student (`student,digit,q,d_c,d_crest,d_brink`), export the
CSV and run:
...
The script derives `d_c` from `q` where a student's slider arithmetic slipped,
then plots each student's `d_crest/d_c` and `d_brink/d_c` against their own `q`
with reference lines at 1.0 and 0.715, and the measured distance from the
Fr = 1 crossing back to the lip in `d_c` units against the textbook 3–4 `d_c`.

![pooled class plot: three bands (d_c/d_c, d_crest/d_c, d_brink/d_c) vs q, plus the critical-position panel](plots/pooled-demo.png)
...
- Hover across the last three or four columns on the lip — the depth falls
  ...
  `d_c` is built on — visible, repeatable, and the reason `d_brink < d_c` is
...
`exercises/FB-2-dc-three-ways/_archive/README-full.md`.

...
Every demo from demo-programme.html, packaged one folder each: a README brief
(lecturer setup · student worksheet · collection & pooled plot · verification
record · director report), collect_plot.py for the Blackboard CSV, the
simulated-class data and plot that prove it works, screenshots, and rig.js
where geometry/instruments are drawn. Statuses and measured caveats:
...
| UN-2 | Flow establishment | UN-2-establishment/ | hammer | (u_max, t_90) |
| UN-3 | Surge tank vs the ODE | UN-3-surge-tank/ | hammer + standpipe | (b_s, y_max, T) |
...
| FR-1 | The friction law | FR-1-friction-law/ | RIG-A | (V, h_f) |
...
| B10 | Lift the crest until the pipe gives up | B10-crest-vs-hgl/ | RIG-A + raised crest | (level, z_sep) |

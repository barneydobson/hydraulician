...
number — the overlay's **measured** normal depth `d_n` — and posts `(q, d_n)`.
...
4. Watch it for ~10 s, take the typical value, and submit **q, d_n**.

## For the instructor — pooling the class

Collect one row per student (`student,digit,scene,q,dn`), export the CSV and
run:
...
The script fits log `d_n` against log `q` and prints the slope against
Manning's 3/5, then histograms every student's back-calculated
`n = d_n^(5/3)·√S₀/q` in a second panel.

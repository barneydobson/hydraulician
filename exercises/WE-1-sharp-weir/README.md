...
4. **Optional — what the plate carries.** The rating counts only the water
   ...
   **Control volume** (`9`) and drag one from about **(2.0, 0.3)** to **(6.7, 1.6)**
   ...
   hydrostatic pressure diagram cut off at the crest,

       F = ρ g P (h − P/2) = ρ g P (H + P/2)

   `F→` sits a few percent above that at every discharge (at q = 0.35: about
   2.9 kN/m against 2.8 computed), and barely moves if you redraw the box.
   ...
   The **Pressure force** tool (pick it on the strip — no digit key) reads
   the same thrust with no box: click the plate itself and it draws that
   hydrostatic-diagram picture directly — the diagram the formula above
   already is. Measured at q = 0.35: **2.78 kN/m**, matching the box's ~2.9
   and the 2.8 computed.
